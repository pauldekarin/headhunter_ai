...
> [!summary] Текущий прогресс (см. таски ниже): **1.1–1.10, 1.12 — done**, **1.11 — partial (нет редактора `llm_deployments`)**. Остался один сквозной гэп: AILayer не пересобирается на PUT /settings — изменение deployments через UI требует рестарта бэка. До MVP-демо осталось закрыть редактор deployments + invalidate-hook.
...
### 1.11 UI: Settings — `S` — **partially done**

Реализовано: `routes/settings/+page.svelte` через **shadcn-svelte Tabs + Superforms + Zod v4 (`zod4` адаптер)** в SPA-режиме (`SPA: true`, `dataType: "json"`, `resetForm: false`). Источник истины — TanStack Query (`settingsQueryKey`), `$effect` синхронит `formData` ← `settings.data`, после PUT — `setQueryData(saved)` без refetch.

Три таба: **search** (`max_pages`, `max_vacancies`), **user** (`auto_submit` switch), **limits** (`hourly_limit`, `daily_limit`, `min_delay_ms`, `delay_jitter_ms`). LLM-секция идёт **passthrough**: `defaultLlm` или кешированное `cached.llm` подкладывается в PUT, чтобы не терять `llm_deployments`/`system_prompt`.

Skeleton при `isPending`, error-state с retry-кнопкой при `isError`. Все строки через Paraglide (`m.settings_*`).

**TODO (отдельные подзадачи)**:
- Редактор `llm_deployments` (add/remove/reorder + поле api_base) — пока в passthrough.
- Резюме (textarea/upload) и `letter_style` редактор — поля есть в схеме settings, UI пока не сделан.
- Pre-MVP: подключить `system_prompt` к UI.

**AC**: Базовый AC выполнен (`PUT /api/v1/settings` работает). **Open gap**: AI Layer **не пересобирается** при обновлении `llm_deployments` — `bootstrap_ai_layer` отрабатывает только на startup (`api/app.py:42-52`). Нужен invalidate-hook в settings route (см. также 1.12).
...
- `AILayer.rebuild(deployments)` существует, но **invalidate-hook на PUT settings ещё не подключён**. На текущий момент Router инициализируется только на startup через `bootstrap_ai_layer` (`api/app.py:42-52`) — изменение `llm_deployments` через UI требует рестарта бэка.
