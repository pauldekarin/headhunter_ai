...
MCP-сервер бэкенда. Позволяет внешним MCP-клиентам (**Claude Desktop**, **Claude Code**, любой совместимый) подключаться и работать с вакансиями. Реализуется в [[Stage 2 - Расширение]] как параллельный канал к внутреннему [[AI Layer]] — даёт «бесплатный» AI для подписчиков Claude Pro/Max (расход покрывается подпиской, не per-token API).
...
## Связи
- [[Description]] — почему MCP, чтобы было «бесплатно» для подписчиков Claude Pro/Max
- [[REST]] — параллельный интерфейс к тем же сущностям
- [[Domain Model]] — какие сущности экспонируются
- [[AI Layer]] — основной канал в [[Stage 1 - MVP]]; MCP — параллельный путь в [[Stage 2 - Расширение]] (задача 2.1)
